...
x0=array(0,dim=c(9,num,500))
truemu0=array(0,dim=c(9,num,500))
k=c(25,50,100);
b=c(3,4,5);
for (time in 1:num){
  ...
  for (i in 1:3){
    for (j in 1:3){
      truemu0[n,time,]=c(rep(0,500-k[i]), rnorm(k[i],rep(b[j],k[i])));
      x0[n,time,]=rnorm(500, truemu0[n,time,]);
      n=n+1;
    }
  }
}
...
#9 situations, 500 replications


MSE=array(0,dim=c(10,9,num));
MAE=array(0,dim=c(10,9,num));

series=seq(1,500,1);
u=seq(from=-8.94,to=9,by=0.06)
thresh=sqrt(2*log(500))
...
save(MSE,MAE,file="/home/youyang4/estimation code/500derror3.RData")
