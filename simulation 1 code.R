...
for (time in 1:num){
  ...
  for (i in 1:4){
    for (j in 1:4){
      truemu=c(rep(0,200-k[i]), rep(b[j],k[i]));
      x0[n,time,]=rnorm(200, truemu);
      n=n+1;
    }
  }
}
...
#10 situations, 500 replications


MSE=array(0,dim=c(10,16,num));
MAE=array(0,dim=c(10,16,num));
...
for (time in 1:num){
  ...
  for (i in 1:4){
    for (j in 1:4){
      ...
      #Johnstone and Silverman, EB Med
...
save(MSE,MAE,file="/home/youyang4/estimation code/200derror.RData")
